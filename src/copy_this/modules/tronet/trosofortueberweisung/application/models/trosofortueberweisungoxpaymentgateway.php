...
class trosofortueberweisungoxpaymentgateway extends trosofortueberweisungoxpaymentgateway_parent
{
    ...
    protected function _getTroPreparedGatewayReason($oOrder, $sGatewayReason)
    {
        ...
        $sGatewayReason = $this->_troReplaceUmlauts($sGatewayReason);

        // Force max gateway length.
        $sGatewayReason = substr($sGatewayReason, 0, 27);
    ...
    protected function _getTransactionPlaceholders($oOrder)
    {
        if (!isset($this->_aTransactionPlaceholder[$oOrder->oxorder__oxid->value]))
        {
            ...
            $this->_aTransactionPlaceholder[$oOrder->oxorder__oxid->value] = array(
