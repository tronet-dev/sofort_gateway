...
class trosofortueberweisung_main extends oxAdminView
{
     ...
     * @since  7.0.0
     */
    private function _troCheckSOFORTUpdates()
    {
        try
        {
            ...
            {
                $aViewData['aMessage']['trosofortueberweisung_update_notification'] = $this->_troRenderUpdateNotificationMessage($oSOFORTRelease);
                $this->addTplParam('aTroMessage', $aViewData);
            }
            else
            {
                $aViewData['aMessage']['trosofortueberweisung_update_notification'] = $this->_troRenderUpdateNotificationMessageLatestVersionInstalled();
                $this->addTplParam('aTroMessage', $aViewData);
            }
        }
        catch (Exception $oException)
        {
            $aViewData['aMessage']['trosofortueberweisung_update_notification'] = $this->_troRenderUpdateNotificationFailedMessage();
            $this->addTplParam('aTroMessage', $aViewData);
        }
    }
     ...
     * @since  7.0.0
     */
    protected function _troRenderUpdateNotificationMessage($oSOFORTRelease)
    {
