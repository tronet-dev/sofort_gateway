...
class trosofortueberweisungcurlutility
{
    ...
    public function __construct($sUrl = null)
    {
        ...
        $this->setErrorCodes($aErrorCodes);
    }
    ...
    {
        $mResult = curl_exec($this->getCh());

        if (curl_errno($this->getCh()) > 0)
        {
            $this->setErrorNumber(curl_errno($this->getCh()));
            $this->setError(curl_error($this->getCh()));
        }
     ...
     * @author  tronet GmbH
     */
    public function setErrorNumber($iErrorNumber)
    {
